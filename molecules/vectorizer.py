...
class SmilesDataGenerator(object):
  ...
  def __init__(self, words, pces, maxlen,
               pad_char=' ',
               pad_min=1,
               pad_weight=0.0,
               test_split=0.20):
  ...
  def sample(self, predicate=None):
    if predicate:
      ...
      pce = self.pces[self.shuffled_word_ixs[word_ix]]
    else:
      idx = [random.choice(np.arange(len(self.pces)))]
      #word = random.choice(self.words)
      word = np.take(self.words, idx)
      pce = np.take(self.pces, idx)
  ...
  def old_sample(self):
    ...
    #idx = random.choice(np.arange(len(words)))
    word = random.choice(self.words)
    #word = np.take(words, idx)
    return np.asarray(self.encode(word))
  ...
  def generator(self, batch_size, sample_func=None):
    while True:
      ...
      pce_tensor = np.zeros((batch_size, ), dtype=np.float32)
      for word_ix in range(batch_size):
        if not sample_func:
          sample_func = self.sample
        word, pce = sample_func()
        data_tensor[word_ix, ...] = self.encode(word)
        pce_tensor[word_ix, ...] = pce
        weight_tensor[word_ix, ...] = self.weight(word)
      yield (data_tensor, pce_tensor, weight_tensor)
